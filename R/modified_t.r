modified_t <- function(x,y,alternative=c("two.sided","less","great")){
  # method2 modified t test
  data = data.frame(x,y)
  ...
  r = tn1.sd/(t1.sd*n1.sd)
    ...
    zt=(n1*d.mean+nh*(t.mean-n.mean))/sqrt(n1*d.sd^2+nh^2*(n.sd^2/n3+t.sd^2/n2))
    
    if (alternative=="two.sided"){
      p.value=2*pnorm(abs(zt), lower.tail = FALSE)  
    }
    else if (alternative=="great"){
      p.value=pnorm(zt, lower.tail=FALSE)
    }
    
    else{
      p.value=pnorm(zt, lower.tail=TRUE)
    }
